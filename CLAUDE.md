...
- `TEN VAD` (default; `--vad=tenvad` or `--vad=subs_then_tenvad`): Low-latency, lightweight, high-accuracy VAD. Requires 16 kHz audio; ffsubsync auto-sets `--frame-rate` to 16000 when selected. Installed by default (dependency: `pip install ten-vad`).
